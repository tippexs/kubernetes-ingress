...
func TestGenerateNginxCfgForAppProtect(t *testing.T) {
	...
	result, warnings := generateNginxCfg(&cafeIngressEx, &apRes, nil, false, configParams, isPlus, false, staticCfgParams, false)
	if diff := cmp.Diff(expected, result); diff != "" {
		t.Errorf("generateNginxCfg() returned unexpected result (-want +got):\n%s", diff)
	}
	if len(warnings) != 0 {
...
func TestGenerateNginxCfgForMergeableIngressesForAppProtect(t *testing.T) {
	...
	result, warnings := generateNginxCfgForMergeableIngresses(mergeableIngresses, &apRes, nil, configParams, isPlus, false, staticCfgParams, false)
	if diff := cmp.Diff(expected, result); diff != "" {
		t.Errorf("generateNginxCfgForMergeableIngresses() returned unexpected result (-want +got):\n%s", diff)
	}
	if len(warnings) != 0 {
